...
// jednoduché Card komponenty
const Card = ({ children }) => (
  <div className="p-4 border rounded-md mb-4 bg-gray-700">{children}</div>
);
    ...
    rows.forEach((row) => {
      ...
      if (status !== undefined) {
        result.statusCounts[status] =
          (result.statusCounts[status] || 0) + 1;
      }

      if (delType) {
        result.deliveryTypes[delType] =
          (result.deliveryTypes[delType] || 0) + 1;
      }
    });

    result.totalToday =
      result.byDay[format(today, "yyyy-MM-dd")].total;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-center items-center gap-4">
        ...
        >
          <FileDown className="w-5 h-5" /> {t[lang].export}
        </button>
      </div>

      {!summary && (
        <p className="text-gray-400">
          Nahraj soubor pro zobrazení dat.
        </p>
      )}
                ...
                <Card key={date}>
                  <CardContent className="p-6 space-y-2">
                    ...
                    <p className="text-gray-200">
                      Zakázek: <strong>{stats.total}</strong>
                    </p>
                    <p className="text-green-400">
                      Hotovo: <strong>{stats.done}</strong>
                    </p>
                    ...
                    <p className="text-gray-200">
                      Palety: <strong>{stats.pallets}</strong>
                    </p>
                    <p className="text-gray-200">
                      Karton: <strong>{stats.cartons}</strong>
                    </p>
                  </CardContent>
                </Card>
              )
